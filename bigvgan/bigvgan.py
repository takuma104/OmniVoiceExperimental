    ...
    def __init__(
        ...
        if activation == "snake":
            self.activations = nn.ModuleList(
                [
                    Activation1d(
                        activation=bigvgan.activations.Snake(
            ...
            self.activations = nn.ModuleList(
                [
                    Activation1d(
                        activation=bigvgan.activations.SnakeBeta(
                            channels, alpha_logscale=h.snake_logscale
                        )
                    )
                    for _ in range(self.num_layers)
...
class BigVGAN(
    ...
    def __init__(self, h: AttrDict, use_cuda_kernel: bool = False):
        ...
        # Post-conv
        activation_post = (
            bigvgan.activations.Snake(ch, alpha_logscale=h.snake_logscale)
            if h.activation == "snake"
            else (
                bigvgan.activations.SnakeBeta(ch, alpha_logscale=h.snake_logscale)
