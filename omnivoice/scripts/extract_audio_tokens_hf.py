...
"""
Extract audio tokens from a HuggingFace Dataset and pack them into
WebDataset shards.

Example usage (Emilia-Dataset, English, streaming):

    python extract_audio_tokens_hf.py \
...
from omnivoice.data.dataset import load_audio_webdataset
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--split",
        required=True,
        help='Dataset split name (e.g. "en", "train").',
    )
    parser.add_argument(
        "--streaming",
        type=str2bool,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of DataLoader workers (keep low for HF streaming).",
    )
    parser.add_argument(
        "--shuffle",
        type=str2bool,
...
class HFDatasetAdapter(IterableDataset):
    """Wraps a HuggingFace (Iterable)Dataset to produce samples in the format
    expected by process_single_sample():
        {"audio": Tensor(1, T), "label": {"id": ..., "audio_duration": ..., ...}}
    """
    ...
    def __iter__(self):
        ...
        for sample in self.hf_dataset:
            ...
            if self.num_machines > 1:
                if global_idx % self.num_machines != self.machine_index:
                    global_idx += 1
                    continue
                global_idx += 1
    ...
    def _convert_sample(self, sample: dict) -> dict | None:
        """Convert an HF dataset sample to the internal format."""
        try:
            ...
            # Build label dict -- try json metadata first, then top-level fields
            json_meta = sample.get("json")
            if isinstance(json_meta, (str, bytes)):
            ...
            sample_id = _get("id", "__key__") or "unknown"
            audio_duration = waveform.shape[1] / self.sample_rate

            label = {
                "id": sample_id,
                "text": _get("text"),
                "language_id": _get("language", "language_id"),
                "audio_duration": audio_duration,
                "speaker": _get("speaker"),
                "dnsmos": _get("dnsmos"),
            }

            # Preserve extra fields from json metadata
            for k, v in json_meta.items():
                if k not in label and k not in ("wav", "mp3", "audio", "__key__", "__url__"):
                    label[k] = v
            ...
            logging.warning(f"Failed to convert HF sample {sample_id}: {e}")
            return None

    def _decode_audio(self, sample: dict) -> torch.Tensor | None:
        """Decode audio from various HF dataset formats."""
        # Case 1: raw audio bytes (mp3, flac, wav) from WebDataset-style streaming
        for audio_key in ("mp3", "flac", "wav", "ogg"):
            ...
            if isinstance(audio_value, bytes):
                return load_audio_webdataset(
                    audio_value, sample_rate=self.sample_rate
                )
            # HF datasets with torchcodec returns an AudioDecoder object
            if hasattr(audio_value, "get_all_samples"):
                audio_samples = audio_value.get_all_samples()
                waveform = audio_samples.data  # (C, T)
        ...
        # Case 2: HF Audio feature (decoded dict with array + sampling_rate)
        audio_info = sample.get("audio")
        if isinstance(audio_info, dict):
            if "array" in audio_info:
                waveform = torch.tensor(
                    audio_info["array"], dtype=torch.float32
                ).unsqueeze(0)
            ...
            if "path" in audio_info and audio_info["path"] is not None:
                ...
                return waveform
        # Case 3: HF Audio feature as AudioDecoder (top-level "audio" key)
        if hasattr(audio_info, "get_all_samples"):
            audio_samples = audio_info.get_all_samples()
        ...
        logging.warning(
            f"No decodable audio found in sample {sample.get('id', '?')}"
        )
        return None
...
def main() -> None:
    ...
    # ── Parse data_files (string or JSON dict) ──
    data_files = args.data_files
    if data_files:
        try:
            data_files = json.loads(data_files)
        except (json.JSONDecodeError, TypeError):
            pass  # keep as plain string glob pattern

    # ── Load HuggingFace dataset ──
    logging.info(
        f"Loading HF dataset: {args.dataset_name}, "
    ...
    if data_files is not None:
        # If data_files is a plain string, wrap as {split: pattern}
    ...
    # Optional shuffle (streaming mode uses buffered shuffle)
    if args.shuffle and args.streaming:
        hf_dataset = hf_dataset.shuffle(
            seed=args.shuffle_seed, buffer_size=args.shuffle_buffer_size
        )

    # ── Estimate total samples (best-effort for progress bar) ──
    total_samples = None
    if not args.streaming:
        total_samples = len(hf_dataset)
        if args.num_machines > 1:
            total_samples = total_samples // args.num_machines
    else:
        try:
            ...
            pass  # indeterminate progress bar

    if total_samples is not None:
        logging.info(f"Estimated total samples: {total_samples}")
    else:
        logging.info("Total samples unknown (streaming mode) -- using indeterminate progress bar")
